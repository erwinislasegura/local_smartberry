...
if (isset($id_dato) && isset($accion_dato) && isset($urlo_dato)) {
    ...
    if($ARRAYPCDESPACHO){
        // Usar método existente para obtener materia prima disponible
        ...
        // Filtrar manualmente los que no tienen PC asignado
        if($ARRAYEXIMATERIAPRIMA){
            $ARRAYEXIMATERIAPRIMA = array_filter($ARRAYEXIMATERIAPRIMA, function($item) {
                // Filtrar solo los que no tienen ID_PCDESPACHO asignado
                return empty($item['ID_PCDESPACHO']) || $item['ID_PCDESPACHO'] === null;
            });
        }
    }
}
                ...
                <div class="container-full">
                    ...
                    <div class="content-header">
                        <div class="d-flex align-items-center">
                            <div class="mr-auto">
                                <h3 class="page-title">Frigorifico MP</h3>
                                <div class="d-inline-block align-items-center">
                                    <nav>
                                        <ol class="breadcrumb">
                                            ...
                                            <li class="breadcrumb-item" aria-current="page">Registro PC MP</li>
                                            <li class="breadcrumb-item active" aria-current="page"> <a href="#"> Seleccion Existencia MP</a>   </li>
                                        </ol>
                                    </nav>
                                </div>
                            </div>
                ...
                if (isset($_REQUEST['SELECIONAREXISTENCIA'])) {
                    ...
                    echo '<script>
                        Swal.fire({
                            ...
                            text:"Se debe selecionar al menos una existencia de materia prima.",
                            showConfirmButton: true,
                            confirmButtonText:"Cerrar",
                            closeOnConfirm:false
                        }).then((result)=>{
                            location.href = "registroSelecionExistenciaMPPcdespachoMP.php?op&id='.$id_dato.'&a='.$accion_dato.'&urlo='.$urlo_dato.'";                            
                        })
                    </script>';
                }
                if ($SINO == "0") {
                    foreach ($SELECIONAREXISTENCIA as $r) :
                        ...
                        // Actualizar directamente el campo ID_PCDESPACHO usando una consulta personalizada
                        $EXIMATERIAPRIMA->__SET('ID_PCDESPACHO', $IDPCDESPACHO);
                        $EXIMATERIAPRIMA->__SET('ID_EXIMATERIAPRIMA', $IDEXIMATERIAPRIMA);
                        //LLAMADA AL METODO DE REGISTRO DEL CONTROLADOR
                        $EXIMATERIAPRIMA_ADO->actualizarSelecionarPCCambiarEstado($EXIMATERIAPRIMA);

                        $AUSUARIO_ADO->agregarAusuario2("NULL",1,2,"".$_SESSION["NOMBRE_USUARIO"].", Se agrego la Existencia de materia prima al PC.","fruta_eximateriaprima", "NULL" ,$_SESSION["ID_USUARIO"],$_SESSION['ID_EMPRESA'], $_SESSION['ID_PLANTA'],$_SESSION['ID_TEMPORADA'] );  
                    ...
                    echo '<script>
                        Swal.fire({
                            ...
                            text:"Se agregado la existencia de materia prima a PC.",
                            showConfirmButton: true,
